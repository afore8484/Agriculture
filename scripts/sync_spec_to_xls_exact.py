...
BACKUP_PATH = BASE / "output" / "doc" / "spec_analysis_draft_v1.before_xls_exact_sync.docx"
...
def section_title(prefix, idx, name):
    return f"{prefix}.{idx} {name}"


def module_summary_text(module):
    sub_names = '?'.join(sub['name'] for sub in module['subs'])
    item_count = sum(len(sub['items']) for sub in module['subs'])
    return f"???? xls ????????{len(module['subs'])}??????{item_count}??????????{sub_names}?"


def module_summary_rows(module):
    item_count = sum(len(sub['items']) for sub in module['subs'])
    return [module['name'], str(len(module['subs'])), str(item_count), '? xls ????????']
...
def main():
    ...
    i32 = find_after(paragraphs, '3.2 ', 70)
    i33 = find_after(paragraphs, '3.3 ', i32 + 1)
    i4 = find_after(paragraphs, '4. ', i33 + 1)
    p32 = paragraphs[i32]
    p33 = paragraphs[i33]
    p4 = paragraphs[i4]

    delete_between(p32, p33)
    delete_between(p33, p4)

    p = insert_paragraph_after(p32, '???????????-??-???????????? 1.xlsx????????????????????????????????????????????????? xls ??????????')
    summary_table = [['????', '?????', '?????', '????']]
    for module in modules:
        summary_table.append(module_summary_rows(module))
    clone_table_after(doc, p, summary_table)

    current_anchor = p
    for idx, module in enumerate(modules, start=1):
        current_anchor = insert_paragraph_after(current_anchor, section_title('3.2', idx, module['name']))
        current_anchor = insert_paragraph_after(current_anchor, module_summary_text(module))

    p = insert_paragraph_after(p33, '??? xls ??????????????????????? xls ????????????????????????????????')
    current_anchor = p
    for idx, module in enumerate(modules, start=1):
        current_anchor = insert_paragraph_after(current_anchor, section_title('3.3', idx, module['name'] + '????'))
        current_anchor = insert_paragraph_after(current_anchor, module_summary_text(module))
        clone_table_after(doc, current_anchor, function_table(module))

    doc.save(DOCX_PATH)
    print('updated')
    print('modules', len(modules))
    print('items', sum(len(sub['items']) for m in modules for sub in m['subs']))
